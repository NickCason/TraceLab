...
describe('SignalCard', () => {
  it('renders the display name', () => {
    const { container } = render(<SignalCard {...makeDefaultProps()} />);
  ...
  it('calls onToggleVisible with the signal index when visibility checkbox is clicked', () => {
    ...
    const { container } = render(<SignalCard {...makeDefaultProps()} onToggleVisible={onToggleVisible} />);
  ...
  it('renders unit label when unit is provided', () => {
    const { container } = render(<SignalCard {...makeDefaultProps()} unit="rpm" />);
    expect(container.textContent).toContain('[rpm]');
  });

  it('renders DIG badge for digital signals', () => {
    const { container } = render(<SignalCard {...makeDefaultProps()} isDigital={true} />);
  ...
  it('renders with reduced opacity when not visible', () => {
    const { container } = render(<SignalCard {...makeDefaultProps()} visible={false} />);
